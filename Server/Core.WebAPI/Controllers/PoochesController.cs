    ...
    public class PoochesController : ControllerBase
    {
        ...
        public async Task<IActionResult> SearchModels()
        {
            try
            {
                ...
                return Ok(_mapper.Map<ICollection<PoochDTO>>(source));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> SearchModel([FromRoute] int id)
        {
            try
            {
                ...
                return Ok(_mapper.Map<PoochDTO>(source));
            }
            catch (Exception ex)
            {
                return StatusCode((int) HttpCode.ServerError, ex.Message);
        ...
        public async Task<IActionResult> SearchModels([FromQuery] SearchParams search)
        {
            try
            {
                ...
                Pagination<PoochModel> source = await _repo.SearchModels(search);
                if (source == null || source?.Results?.Count == 0)
                    return NoContent();

                Response.SetPagination(source);
                return Ok(_mapper.Map<ICollection<PoochDTO>>(source.Results));
            }
        ...
        [HttpPut]
        public async Task<IActionResult> CreateModel([FromBody] PoochDTO model)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                var source = await _repo.CreateModel(_mapper.Map<PoochModel>(model));
                return Ok(_mapper.Map<PoochDTO>(source));
            }
            catch (ModelStateException)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateModel([FromBody] PoochDTO model)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                var source = await _repo.UpdateModel(_mapper.Map<PoochModel>(model));
                return Ok(_mapper.Map<ICollection<PoochDTO>>(source));
            }
            catch (ModelStateException)
            {
